...
import { JournalEntry, getColorClass } from '@/types/journal';
        ...
        {sortedEntries.length === 0 ? (
          ...
          <div className="py-2">
            {sortedEntries.map((entry) => {
              ...
              // Extract the base color (e.g., 'red' from 'bg-red-100')
              ...
              return (
                <div
                  ...
                  className={`group relative px-4 py-3 cursor-pointer transition-colors border-l-4 ${
                    selectedId === entry.id
                      ? `bg-white border-l-${baseColor}-600`
                      : `border-l-transparent hover:bg-white hover:border-l-${baseColor}-300`
                  }`}
                ...
                >
                  <div className="flex items-start gap-3">
                    {/* Color indicator dot */}
                    <div
                      className={`flex-shrink-0 mt-1 w-3 h-3 rounded-full ring-2 ring-white ${colorClass.split(' ').slice(0, 1).join(' ')}`}
                      title={baseColor}
                    />
